...
def generate_drawdown_over_time_data(trades, account_config):
    ...
    for trade in sorted_trades:
        ...
        drawdown_percent = (
            (drawdown_dollars / peak_equity * 100) if peak_equity > 0 else 0
        )
...
def generate_inception_drawdown_data(trades, account_config):
    ...
    final_return = (
        inception_drawdown_percentages[-1] if inception_drawdown_percentages else 0
    )
    if final_return >= 0:
...
def generate_time_of_day_performance_data(trades):
    ...
    other_sessions = sorted(
        [s for s in session_stats.keys() if s not in TRADING_SESSION_ORDER]
    )
...
def filter_and_aggregate_by_timeframe(
    ...
    def get_bucket_key(date, interval):
        ...
        if interval == "30min":
            ...
            return date.replace(
                minute=(date.minute // 30) * 30, second=0, microsecond=0
            )
        elif interval == "daily":
...
def generate_portfolio_value_charts(trades, account_config):
    ...
    def create_day_chart_data(sorted_trades, end_date, base_value):
        ...
        day_trades = [
            ...
            if parse_trade_datetime(t)
            and start_date <= parse_trade_datetime(t) <= end_date
        ]
    ...
    for timeframe in timeframes:
        if timeframe == "day":
            ...
            chart_data = create_chart_data(
                trade_dates, portfolio_values, timeframe, interval, end_date, base_value
            )
        ...
        print(
            f"  ✓ Portfolio value ({timeframe}) with {get_default_interval(timeframe)} interval saved"
        )
...
def generate_total_return_charts(trades, account_config):
    ...
    def create_day_chart_data(sorted_trades, end_date, starting_investment):
        ...
        if portfolio_value_at_day_start == 0:
            portfolio_value_at_day_start = (
                starting_investment if starting_investment > 0 else 1
            )
    ...
    for timeframe in timeframes:
        if timeframe == "day":
            # Special handling for day timeframe
            chart_data = create_day_chart_data(
                sorted_trades, end_date, starting_investment
            )
        else:
            # Normal cumulative return for other timeframes
            interval = get_default_interval(timeframe)
            chart_data = create_chart_data(
                trade_dates, return_percentages, timeframe, interval, end_date
            )
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            json.dump(chart_data, f, indent=2)
        print(
            f"  ✓ Total return ({timeframe}) with {get_default_interval(timeframe)} interval saved"
        )
...
def main():
    ...
    save_json_file(
        "index.directory/assets/charts/performance-by-day-data.json", day_data
    )
    ...
    save_json_file(
        "index.directory/assets/charts/ticker-performance-data.json", ticker_data
    )
    ...
    save_json_file(
        "index.directory/assets/charts/drawdown-over-time-data.json", drawdown_data
    )
    ...
    save_json_file(
        "index.directory/assets/charts/inception-drawdown-data.json", inception_dd_data
    )
